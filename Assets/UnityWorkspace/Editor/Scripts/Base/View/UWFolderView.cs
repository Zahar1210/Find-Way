...
public class UWFolderView : UWFileView<UWFolder>
{
    private ScrollView _scrollView;
    private ToolbarSearchField _searchField;
    private Slider _scaleSlider;
    
    private UWAssetContainer[] _assetContainers = Array.Empty<UWAssetContainer>();
    ...
    private float AssetMarginHorizontal => 3f;
    private float AssetMarginVertical => 5f;
    private float AssetPaddingTop => 5f;
    private float AssetMinSize => 0.5f;
    private float ContainerPadding => 5f;
    ...
    public UWFolderView(UWFolder file) : base(file)
    {
        File.UpdateAssets();
    }
    ...
    {
        LoadUXML(UWContainer.Instance.FolderUXML);
    }
    ...
    protected override void RegisterCallbacks()
    {
        ...
        _scaleSlider.RegisterValueChangedCallback(OnScaleValueChanged);
    }
    ...
    {
        VisualTreeAsset uxml = LineMode ? UWContainer.Instance.FolderAssetContainerLineUXML : UWContainer.Instance.FolderAssetContainerUXML;
        if (_currentAssetContainerUXML != uxml)
        {
            _currentAssetContainerUXML = uxml;
            
            foreach (UWAssetContainer assetContainer in _assetContainers)
            {
                assetContainer.HoverUpdated -= OnUpdateAssetHover;
                assetContainer.parent.parent.RemoveFromHierarchy();
            }
            
            _assetContainers = Array.Empty<UWAssetContainer>();
        }
        ...
        _assetContainers = _scrollView.Query<UWAssetContainer>().ToList().ToArray();
    ...
    private void OnScaleValueChanged(ChangeEvent<float> evt)
    {
        float scale = evt.newValue;
        if (evt.newValue < AssetMinSize)
        {
            scale = Mathf.RoundToInt(evt.newValue / AssetMinSize) * AssetMinSize;
            
            _scaleSlider.UnregisterValueChangedCallback(OnScaleValueChanged);
            _scaleSlider.value = scale;
            _scaleSlider.RegisterValueChangedCallback(OnScaleValueChanged);
        }
        
        File.AssetScale = scale;
